...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    simp_rnn = GRU(units, return_sequences=True, 
                 implementation=2, name='rnn1')(input_data)
    bn_rnn = BatchNormalization()(simp_rnn)
    for i in range(recur_layers-1):
        simp_rnn = GRU(units, return_sequences=True, 
                     implementation=2, name='rnn' + str(i+2))(bn_rnn)
        bn_rnn = BatchNormalization()(simp_rnn)        
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(bn_rnn)
    # Add softmax activation layer
...
def final_model(input_dim, units, filters, kernel_size, conv_stride,
    ...
    # Add a recurrent layer
    simp_rnn1 = GRU(units, return_sequences=True, 
                 implementation=2, name='rnn1')(dropout1)
    bn_rnn = BatchNormalization()(simp_rnn1)    
    simp_rnn2 = GRU(int(units/2), return_sequences=True, 
